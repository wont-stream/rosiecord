...
class Shell {
  ...
  static async run(
    command: string = "ls",
    after?: (stderr: ExecException | null, stdout: string) => any
  ): Promise<string> {
    return await new Promise((resolve): void => {
  ...
  static async runSilently(
    command: string = "ls",
    after = (stderr: ExecException | null, stdout: string): void => {}
  ): Promise<string> {
    return await new Promise((resolve): void => {
...
class Constants {
  static IPA_FETCH_LINK =
    "https://ipa.aspy.dev/discord/stable/Discord_227.0.ipa";
}
